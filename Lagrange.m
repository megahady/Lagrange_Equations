...
% l=0.5;
% L=0.5*(M+m)+dx^2+ m*dx*l*dtheta*cos(theta)+0.5*m*l^2*dtheta^2+m*g*l*cos(theta)        %Define the Lagragian.
% Equations=Lagrange(L,[theta,dtheta,ddtheta,x,dx,ddx]) 
% returns   [m*l*(ddx*cos(theta)+l*ddtheta+g*sin(theta)),2*ddx+m*l*ddtheta*cos(theta)-m*l*dtheta^2*sin(theta)]
...
Vt=V;
    for cont0=1:1:Var
        Vt(3*cont0-2)=strcat('f',num2str(cont0),'(t)');
